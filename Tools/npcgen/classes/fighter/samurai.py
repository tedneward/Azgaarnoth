...
def level7(npc):
    npc.features.append("**Elegant Courtier.** Whenever you make a Charisma (Persuasion) check, you gain a bonus to the check equal to your Wisdom modifier.")
    if "WIS" in npc.savingthrows:
        npc.savingthrows.append(choose("Choose to gain proficiency in either INT or CHA saving throws:", ["INT", "CHA"]))
    else:
        npc.savingthrows.append("WIS")

def level10(npc):
    npc.features.append("**Tireless Spirit.** When you roll initiative and have no uses of Fighting Spirit remaining, you regain one use.")
    replace("**Fighting Spirit.**", npc.bonusactions, "**Fighting Spirit.** You can give yourself advantage on all weapon attack rolls until the end of the current turn. When you do so, you also gain 10 temporary hit points.\nYou can use this feature three times. You regain all expended uses of it when you finish a long rest.")

def level15(npc):
    npc.actions.append("**Rapid Strike.** If you take the Attack action on your turn and have advantage on an attack roll against against one of the targets, you can forgo the advantage for that roll to make an additional weapon attack against that target, as part of the same action. You can do so no more than once per turn.")
    replace("**Fighting Spirit.**", npc.bonusactions, "**Fighting Spirit.** You can give yourself advantage on all weapon attack rolls until the end of the current turn. When you do so, you also gain 15 temporary hit points.\nYou can use this feature three times. You regain all expended uses of it when you finish a long rest.")
